...
import { Compass, Users, TrendingUp, Sparkles, ArrowRight, Play, Globe, Menu, X, Star, CheckCircle2, ChevronRight, Search, MapPin, Utensils, Plane, Calendar, ShieldCheck, Zap } from 'lucide-react';
import { Button, GlassCard } from './UI';
import { EXPERIENCES, TESTIMONIALS, STATS, LOGOS, Experience } from '../constants';
export function Navbar({ onBook }: { onBook: () => void })

export function ExperienceDetailModal({ exp, isOpen, onClose }: { exp: Experience | null, isOpen: boolean, onClose: () => void }) {
  if (!exp) return null;
  ...
  🚀 Book Now
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
              ...
              <div className="p-8 md:p-12 overflow-y-auto max-h-[70vh] lg:max-h-none">
                ...
                <p className="text-white/60 mb-8 leading-relaxed font-light">{exp.description}</p>
                ...
                <div className="grid grid-cols-2 gap-6 mb-10">
                   <div className="glass-immersive p-4 rounded-2xl flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] uppercase text-white/40 tracking-widest">Duration</p>
                   ...
                   <div className="glass-immersive p-4 rounded-2xl flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] uppercase text-white/40 tracking-widest">Meals</p>
                ...
                <div className="mb-10">
                  <h3 className="text-lg font-bold mb-6 flex items-center gap-3">
                    <Plane className="w-5 h-5 text-accent-emerald" /> Trip Itinerary
                  </h3>
                  <div className="space-y-6">
                    {exp.itinerary.length > 0 ? exp.itinerary.map((item, idx) => (
                      <div key={idx} className="relative pl-8 border-l border-white/10 py-1">
                        <div className="absolute top-2 -left-[5px] w-2.5 h-2.5 rounded-full bg-accent-emerald shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                        <span className="text-[10px] uppercase font-bold text-accent-emerald block mb-1">Day {item.day}</span>
                        <h4 className="font-bold text-white mb-2">{item.title}</h4>
                        <p className="text-sm text-white/50 leading-relaxed mb-3">{item.desc}</p>
                        <div className="flex gap-2">
                          {item.meals.map((meal, mIdx) => (
                            <span key={mIdx} className="text-[9px] bg-white/5 border border-white/5 px-2 py-0.5 rounded-md text-white/40 font-bold uppercase tracking-tighter">
                              {meal}
                            </span>
                          ))}
                        </div>
                      ...
                      <p className="text-white/30 italic text-sm">Detailed day-wise plan coming soon. Contact us for custom Patna-departure routes.</p>
                    )}
                  </div>
                </div>

                <Button className="w-full py-5 text-sm font-black uppercase tracking-[0.2em]">🚀 Request Booking Details</Button>
              </div>
            </div>
          </motion.div>
        </motion.div>
...
export function PastExpeditions() {
  const pastEvents = [
    { title: 'Meta Leadership Retreat', location: 'Bali, 2025', image: 'https://images.unsplash.com/photo-1511795409834-ef04bbd61622?auto=format&fit=crop&q=80&w=2070', stats: '25 Leaders' },
    { title: 'Google Innovation Sprints', location: 'Switzerland, 2024', image: 'https://images.unsplash.com/photo-1517457373958-b7bdd4587205?auto=format&fit=crop&q=80&w=2070', stats: '40 Engineers' },
    { title: 'Airbnb Founders Summit', location: 'Norway, 2024', image: 'https://images.unsplash.com/photo-1540317580384-e5d43616b9aa?auto=format&fit=crop&q=80&w=2070', stats: '15 High-impact' }
  ];
    ...
    <section className="py-32 relative overflow-hidden bg-premium-black">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <div>
            <h2 className="text-[10px] uppercase tracking-[0.3em] font-bold text-accent-emerald mb-4">Past Memories</h2>
            <h3 className="text-4xl md:text-5xl font-bold tracking-tight">Success Stories</h3>
          </div>
          <p className="text-white/30 max-w-sm mb-4 font-medium leading-relaxed">
            A glimpse into the transformations we've hosted for global innovators.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {pastEvents.map((event, idx) => (
            <div key={idx} className="group relative h-[400px] rounded-[2rem] overflow-hidden">
               <img src={event.image} alt={event.title} className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer" />
               <div className="absolute inset-0 bg-gradient-to-t from-premium-black to-transparent" />
               <div className="absolute bottom-8 left-8">
                 <p className="text-[10px] font-bold text-accent-emerald uppercase tracking-widest mb-2">{event.location}</p>
    ...
    <nav className={`fixed top-0 left-0 right-0 z-[999] transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto h-full px-6 md:px-10 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-8">
          {['Explore', 'Corporate', 'Community', 'About', 'Resources'].map((item) => (
            <a
              key={item}
          ...
          <a href="#contact" className="text-[10px] uppercase text-white/60 hover:text-accent-emerald">
            Contact
          </a>
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-32 pb-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-20 text-center">
        ...
        >
          From the heights of Darjeeling to the Fjords of Norway. 
          Premium travel experiences tailored for leaders, creators, and world-shakers.
        </motion.p>
        ...
        >
          ...
          <Button variant="outline" className="w-full sm:w-auto px-10 py-5 text-lg group">
        ...
        >
          {[
            ...
            <div key={idx} className="glass-morphism p-4 rounded-2xl flex flex-col items-center">
              <span className="text-2xl font-display font-bold text-accent-pink">{stat.value}</span>
...
export function ExperienceShowcase({ onSelectExp }: { onSelectExp?: (exp: Experience) => void }) {
  ...
  return (
    <section id="explore" className="py-32 relative overflow-hidden bg-premium-black">
      
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-8">
          <div className="max-w-2xl">
            <h2 className="text-[10px] uppercase tracking-[0.3em] font-bold text-accent-emerald mb-4">The Collection</h2>
            <h3 className="text-4xl md:text-6xl font-bold leading-tight tracking-tight mb-8">Curated Expeditions</h3>
          ...
          <p className="text-white/30 max-w-sm mb-4 font-medium leading-relaxed bg-white/5 p-6 rounded-2xl border border-white/5">
            Discover breathtaking landscapes and high-impact communities. 
            All plans include luxury stays, gourmet meals, and expert-led workshops.
          </p>
        </div>
              ...
              >
                ...
                <div className="absolute bottom-8 left-8 right-8 z-10">
                  <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                      <p className="text-accent-emerald text-[11px] uppercase tracking-[0.3em] font-bold mb-2">Location: {exp.location}</p>
                      <h4 className="text-3xl md:text-4xl font-bold mb-2 tracking-tight">{exp.title}</h4>
                    </div>
                    <div className="bg-white/10 backdrop-blur-2xl border border-white/10 p-6 rounded-3xl flex flex-col items-center min-w-[160px] group-hover:bg-accent-emerald transition-colors duration-500">
                      <span className="text-[10px] uppercase tracking-widest font-bold text-white/60 group-hover:text-white/80">Starts At</span>
                      <span className="text-2xl font-black tracking-tight self-center mt-1">
                        ₹{exp.pricePerHead.toLocaleString()}
                      </span>
                      <span className="text-[8px] uppercase tracking-widest font-bold text-white/40 group-hover:text-white/60">Per Person</span>
                    </div>
                  </div>
                </div>
              </motion.div>
            ...
            >
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/50 uppercase tracking-widest">VP of People, Meta</p>
                </div>
              </div>
              <p className="text-sm italic text-white/80 leading-relaxed">
                "The ROI on an Aetheris offsite isn't just better culture—it's visible in our revenue growth."
              </p>
            </motion.div>
          </div>

          <div>
            <h2 className="text-xs uppercase tracking-[0.3em] font-bold text-accent-emerald mb-6">Corporate Solutions</h2>
            <h3 className="text-5xl md:text-6xl font-bold leading-tight mb-8">Offsites Designed for High-Scale Teams</h3>
            
            <div className="space-y-8 mb-12">
              {[
                { title: 'Leadership Retreats', desc: 'Strategic alignment in the world’s most inspiring boardrooms.', icon: Compass },
                { title: 'Team Sprints', desc: 'Focus on shipping while surrounded by breathtaking nature.', icon: Users },
                { title: 'Cultural Resets', desc: 'Deep bonding experiences that eliminate silos forever.', icon: Sparkles }
              ].map((item, idx) => (
            ...
            <Button className="px-10 py-5 text-lg">Download Executive Guide</Button>

            <div className="mt-16 pt-8 border-t border-white/10">
              <p className="text-[10px] uppercase tracking-widest text-white/30 mb-6">Trusted by world-changing companies</p>
              <div className="flex flex-wrap gap-x-8 gap-y-4 opacity-40 grayscale group-hover:grayscale-0 transition-all">
                {LOGOS.map(logo => (
                  <span key={logo} className="text-lg font-bold tracking-tighter hover:opacity-100 cursor-default">{logo}</span>
                ))}
              </div>
            </div>
          </div>
    ...
    <section id="community" className="py-32 relative">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-20">
          <h2 className="text-xs uppercase tracking-[0.3em] font-bold text-accent-emerald mb-4">Network Effect</h2>
          <h3 className="text-5xl md:text-6xl font-bold leading-tight max-w-3xl mx-auto">More Than a Trip. A Lifelong Network.</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <GlassCard className="col-span-1 md:col-span-2 p-12">
            <div className="flex flex-col md:flex-row gap-12 items-center">
              <div className="flex-1">
                <h4 className="text-3xl font-display font-bold mb-6">Expert-Led Mentorship</h4>
                <p className="text-white/60 mb-8 font-light leading-relaxed text-lg">
                  Every expedition features sessions with billion-dollar founders and world-class psychologists 
                  to help you rewire your thinking.
                </p>
            <div className="flex -space-x-4 mb-8">
                  {[1,2,3,4,5].map(i => (
                    <img key={i} src={`https://i.pravatar.cc/150?u=${i*10}`} alt="Expert" className="w-14 h-14 rounded-full border-4 border-premium-black" />
                  ))}
              ...
              <div className="flex-1 w-full relative">
                 ...
                 <img src="https://images.unsplash.com/photo-1517486808906-6ca8b3f04846?auto=format&fit=crop&q=80&w=2070" className="rounded-2xl relative z-10 w-full" alt="Workshop" referrerPolicy="no-referrer" />
              </div>
            </div>
          </GlassCard>
          ...
          <GlassCard className="p-12 flex flex-col justify-between">
            <div>
              ...
              <h4 className="text-2xl font-display font-bold mb-4">High-Stakes Networking</h4>
              <p className="text-white/60 font-light text-sm leading-relaxed mb-8">
                Skip the small talk. Engage in deep-meaning conversations through curated dinner themes and collaborative challenges.
              </p>
            </div>
            <div className="space-y-4">
               {[
                 ...
                 <div key={idx} className="flex justify-between items-center py-2 border-b border-white/5">
                   <span className="text-xs uppercase tracking-widest text-white/40">{item.label}</span>
        ...
        <div className="mt-20">
          <div className="flex flex-wrap items-center justify-center gap-6">
            {TESTIMONIALS.map((t) => (
              <GlassCard key={t.id} className="w-full md:w-[400px] p-8" hover={false}>
                 <div className="flex items-center gap-1 text-accent-orange mb-6">
                    {[1,2,3,4,5].map(i => <Star key={i} className="w-4 h-4 fill-current" />)}
                 </div>
                 <p className="text-lg font-light leading-relaxed mb-8">"{t.content}"</p>
...
export function HowItWorks() {
  const steps = [
    { num: '01', title: 'Discover', desc: 'Browse our curated catalog of global transformation hubs.' },
    { num: '02', title: 'Connect', desc: 'Complete your profile and match with like-minded travelers.' },
    { num: '03', title: 'Experience', desc: 'Embark on a high-end voyage led by elite facilitators.' },
    { num: '04', title: 'Transform', desc: 'Return with a new network, skills, and strategic clarity.' },
  ];
  ...
  return (
    <section className="py-32 relative overflow-hidden bg-white/5">
       <div className="max-w-7xl mx-auto px-6">
          <div className="mb-20">
             <h2 className="text-xs uppercase tracking-[0.3em] font-bold text-accent-emerald mb-4">The Journey</h2>
             <h3 className="text-4xl md:text-5xl font-bold">Simple Process. <br /> Profound Results.</h3>
          </div>
      ...
      <div className="max-w-7xl mx-auto glass-immersive rounded-2xl p-10 flex flex-col lg:flex-row items-center justify-between gap-12 backdrop-blur-2xl">
          ...
          {STATS.map((stat, idx) => (
             <div key={idx} className="flex flex-col gap-1">
                <div className="text-3xl font-bold flex items-baseline">
                   ...
                   <span className="text-accent-red text-xl ml-0.5">{stat.suffix}</span>
                </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-10 opacity-30 grayscale invert brightness-200">
           {LOGOS.slice(0, 4).map(logo => (
             <span key={logo} className="text-xl font-black tracking-tighter uppercase">{logo}</span>
           ))}
        </div>
      </div>
    </section>
  ...
  return (
    <section className="py-32 relative">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div>
            <h2 className="text-4xl md:text-5xl font-bold mb-8 leading-tight">Ready to Upgrade <br />Your Life & Team?</h2>
            <p className="text-white/60 font-light text-lg mb-12 leading-relaxed">
              Join the waiting list for our Summer 2026 expeditions or schedule a custom 
              consultation for your corporate leadership team.
            </p>
            
            <div className="space-y-6">
               <div className="flex items-center gap-4">
                  <CheckCircle2 className="w-6 h-6 text-accent-emerald" />
                  <span className="text-white/80">Exclusive early-bird access to destinations</span>
               </div>
               <div className="flex items-center gap-4">
                  <CheckCircle2 className="w-6 h-6 text-accent-emerald" />
                  <span className="text-white/80">Personal onboarding with our Experience Leads</span>
               </div>
            </div>
          </div>
            ...
            <form className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-[0.1em] text-white/40 font-bold">Full Name</label>
                  <input type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-accent-pink transition-colors" placeholder="John Doe" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-[0.1em] text-white/40 font-bold">Email Address</label>
                  <input type="email" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-accent-pink transition-colors" placeholder="john@example.com" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs uppercase tracking-[0.1em] text-white/40 font-bold">I am interested in...</label>
                <select className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-accent-pink transition-colors appearance-none">
                   <option className="bg-premium-black">Personal Expedition</option>
                   <option className="bg-premium-black">Corporate Offsite</option>
                   <option className="bg-premium-black">Mentorship Program</option>
                </select>
              </div>

              <div className="space-y-2">
                <label className="text-xs uppercase tracking-[0.1em] text-white/40 font-bold">Message (Optional)</label>
                <textarea className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-accent-pink transition-colors h-32" placeholder="Tell us about your goals..." />
              </div>
    ...
    <footer className="py-20 border-t border-white/5 bg-premium-black">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          <div className="space-y-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xl font-display font-bold tracking-tight">AETHERIS</span>
            </div>
            <p className="text-white/40 text-sm leading-relaxed max-w-xs">
              The world's premier platform for hybrid travel and corporate transformation. 
              Built for those who demand excellence in every aspect of life.
            </p>
          </div>

          {[
            { title: 'Platform', links: ['Experiences', 'Corporate', 'Community', 'Pricing'] },
            { title: 'Company', links: ['About Us', 'Careers', 'Brand Kit', 'Contact'] },
            { title: 'Legal', links: ['Privacy Policy', 'Terms of Service', 'Cookie Policy'] }
          ].map((col, idx) => (
            <div key={idx}>
              <h5 className="font-bold mb-6 text-xs uppercase tracking-[0.2em]">{col.title}</h5>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between pt-8 border-t border-white/5 gap-8">
          <p className="text-white/20 text-xs">© 2026 Aetheris Global Expeditions. All rights reserved.</p>
